...
export default class TimelinePlugin {
  ...
  constructor(params, _ws) {
    ...
    _defineProperty(this, "_onRedraw", () => this.render());

    _defineProperty(this, "_onReady", () => {
      const ws = this.wavesurfer;
      this.drawer = ws.drawer;
      this.pixelRatio = ws.drawer.params.pixelRatio;
      this.maxCanvasWidth = ws.drawer.maxCanvasWidth || ws.drawer.width;
      this.maxCanvasElementWidth = ws.drawer.maxCanvasElementWidth || Math.round(this.maxCanvasWidth / this.pixelRatio); // add listeners
    ...
    _defineProperty(this, "_onWrapperClick", e => {
      e.preventDefault();
    ...
    this._onZoom = this.params.zoomDebounce ? this.wavesurfer.util.debounce(() => this.render(), this.params.zoomDebounce) : () => this.render();
  }
  /**
  ...
  updateCanvasesPositioning() {
    ...
    this.canvases.forEach((canvas, i) => {
      ...
      // required width
      const canvasWidth = i === canvasesLength - 1 ? this.drawer.wrapper.scrollWidth - this.maxCanvasElementWidth * (canvasesLength - 1) : this.maxCanvasElementWidth; // set dimensions and style
  ...
  renderCanvases() {
    ...
    const formatTime = this.params.formatTimeCallback; // if parameter is function, call the function with
    ...
    let curSeconds = 0;
    let i; // build an array of position data with index, second and pixel data,
